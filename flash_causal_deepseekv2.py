...
class FlashDeepseekv2ForCausalLM(FlashForCausalLM):
    ...
    def init_h3p_config(self, config, ep_level=ExpertParallelDegree.NO_EP):
        ...
        if config is None:
            ...
            self.enable_lcoc_all2all = False if (ep_level == ExpertParallelDegree.STATIC_EP) \
    ...
    def init_ascend_weight(self):
        ...
        coder_param = {
            ...
            "enableTopkOutput": ENV.enable_expert_hotpot_gather and ENV.expert_hotpot_dump_path is not None,
        ...
        if self.prefix_cache_enable and q_lens:
            ...
            if sum(prefix_lens) != 0:
                ...
                self.in_history_compressed_kv = torch.empty([sum(prefix_lens), self.kv_lora_rank],
                                                                dtype=kv_cache[0][0].dtype).pin_memory()
    ...
    def prepare_paddingidx_for_densetpparallel(self, input_lengths, is_prefill, **kwargs):
        ...
        if token_size_per_dp_group is None and not is_prefill:
            ...
            print_log(self.tp_rank, logger.info, f"===Create token_size_per_dp_group:{token_size_per_dp_group}===")
        tp_rank = self.mapping.attn_tp.rank
        tmp_dp_padding_idx = []
    ...
    def prepare_inputs_for_ascend(self,
        ...
        if self.mix_shared_routing:
            if self.ep_level == ExpertParallelDegree.STATIC_EP:
                ...
                if is_prefill and self.enable_gating_dp:
                    self.mix_shared_routing_weight = torch.tensor([1] * (self.atom_dp_size), \
                                                                dtype=torch.float).view(self.atom_dp_size, 1).npu()
                else:
        ...
        if is_prefill:
            ...
            if self.eplb_level in [EPLBType.STATIC_EPLB, EPLBType.DYNAMIC_EPLB]:
                ...
                EplbExpertDataCollect().collect_routing_map(self.expert_routing_map)
            if self.mix_shared_routing:
            ...
            if self.mapping.has_attn_cp():
                ...
                lm_head_indices = torch.arange(input_ids.size(0) * self.mapping.attn_cp.group_size,
                                               dtype=torch.int64, device=input_ids.device)
    ...
    def forward(
        ...
        if not is_prefill and self.mapping.has_attn_cp():
            ...
            logits = logits[:input_ids.size(0)]
    ...
    def forward_mtp(
        ...
        if is_prefill:
            ...
            if is_mtp:
                ...
                logits, hidden_states = self.execute_ascend_operator(acl_inputs, acl_param, is_prefill)
                if self.mapping.has_attn_cp():
                    # During the CP decode stage, each CP domain processes the same input token,
                    # leading to identical output tokens. Consequently, duplicates occur in the aggregated next_token.
                    logits = logits[:input_ids.size(0)]
