...
class EplbLoaderProcess():
    def __init__(self, weights: Weights, device):
        self.is_alive = True
        ...
        self.weight_column_linear_process \
            = spawn_ctx.Process(target=EplbLoaderProcess._do_load_weight_column_linear_from_ssd,
                                args=(weights, device, self.weight_column_linear_in, self.weight_column_linear_out))
        ...
        self.weight_row_linear_process = (
            spawn_ctx.Process(target=EplbLoaderProcess._do_load_weight_row_linear_from_ssd,
                              args=(weights, device, self.weight_row_linear_in, self.weight_row_linear_out))
        )
        self.weight_row_linear_process.start()

    @staticmethod
    def _do_load_weight_column_linear_from_ssd(weights, device, in_q, out_q):
    ...
    @staticmethod
    def _do_load_weight_row_linear_from_ssd(weights, device, in_q, out_q):
        torch_npu.npu.set_device(device)
        weights.switch_process_group(ProcessGroupType.MOE_EP)
